...
regressiondat=read.csv('~/Documents/Molly Hayes/SFSU/Thesis/Molly r code and data/All bee data/Resilience regression 2002 no abs.csv',header=TRUE)
head(regressiondat)
library(ggplot2)
...
Wconnectanceplotbsr=ggplot(Wconnectance,aes(Wconnectance$Before,bee.div$Change))+xlab("Before")+ylab("Change")+geom_point()+geom_smooth(method='lm', se=FALSE)+ggtitle("Weighted connectance")+theme(plot.title = element_text(hjust = 0.5))
Wconnectanceplotbsr
...
#Wconnectancebeedivplot=ggplot(aes(Wconnectance$Before,bee.div$Change))+geom_point()+geom_smooth(method='lm', se=FALSE)+ggtitle("Weighted connectance")+theme(plot.title = element_text(hjust = 0.5))
#Wconnectancebeedivplot
