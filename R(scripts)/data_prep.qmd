...
model1 <- lmer(no_spp ~ year * treatment + sampling_events + (1 | site), data = sites_summary)

# Summary of the model
summary(model1)

# Extract p-values
p_values <- summary(model1)$coefficients[, "Pr(>|t|)"]
...
# Fit a mixed-effects model
model2 <- lmer(abund ~ year * treatment + sampling_events + (1 | site), data = sites_summary)
...
emm1 <- emmeans(model1, ~ year * treatment)
plot(emm1)
...
emm2 <- emmeans(model2, ~ year * treatment)
...
plot_abund
```
###Analyze using a poisson regression and negative binomial model

Re-analyze this as a poisson regression because it is count data
